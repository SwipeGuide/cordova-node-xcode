 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.addSupportingFile = function(path, opt, group) {
    opt = opt || {};
    ...
    if (!opt.plugin) {
        correctForSupportingPath(file, this);
        file.fileRef = this.generateUuid();
    }
    ...
    if (!opt.plugin) {
        ...
        if (group) {
            ...
            this.addToSupportingPbxGroup(file);          // PBXGroup
        }
 ...
 * @returns {Object} file; see pbxFile
 */
pbxProject.prototype.removeSupportingFile = function(path, opt, group) {
    var file = new pbxFile(path, opt);
    file.target = opt ? opt.target : undefined;

    correctForSupportingsPath(file, this);
    ...
    if (group) {
        ...
        this.removeFromSupportingPbxGroup(file);          // PBXGroup
    }
    this.removeFromPbxResourcesBuildPhase(file);     // PBXResourcesBuildPhase
...
pbxProject.prototype.addToSupportingPbxGroup = function(file) {
    var pluginsGroup = this.pbxGroupByName('Supporting');
    if (!pluginsGroup) {
        this.addPbxGroup([file.path], 'Supporting');
    } else {
        pluginsGroup.children.push(pbxGroupChild(file));
    }
}

pbxProject.prototype.removeFromSupportingPbxGroup = function(file) {
    if (!this.pbxGroupByName('Supporting')) {
        return null;
    }
    var pluginsGroupChildren = this.pbxGroupByName('Supporting').children, i;
    for (i in pluginsGroupChildren) {
        if (pbxGroupChild(file).value == pluginsGroupChildren[i].value &&
            pbxGroupChild(file).comment == pluginsGroupChildren[i].comment) {
            pluginsGroupChildren.splice(i, 1);
...
function correctForSupportingPath(file, project) {
    return correctForPath(file, project, 'Supporting');
}
